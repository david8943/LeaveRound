...
public class DandelionDonationServiceImpl implements DandelionDonationService {
    ...
    public void donateDandelions(Integer userId, DandelionDonationRequestDTO donationRequestDTO) {
        ...
        int goldCount = donationRequestDTO.getGoldDandelionCount() != null ? donationRequestDTO.getGoldDandelionCount() : 0;
        ...
        // 기부 가능한 민들레 개수 확인 - 보유 중인 개수로 직접 확인 (use count는 무시)
        int availableNormal = Math.max(0, user.getDandelionCount());
        int availableGold = Math.max(0, user.getGoldDandelionCount());
